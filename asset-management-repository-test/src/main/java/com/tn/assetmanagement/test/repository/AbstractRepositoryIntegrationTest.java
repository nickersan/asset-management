...
@TestPropertySource(properties = "spring.datasource.url=jdbc:oracle:thin:@localhost:${oracle.port}:XE")
@Testcontainers
public class AbstractRepositoryIntegrationTest
{
  ...
  private static final String DIRECTORY_HOST_ORACLE_DATA = "/var/lib/docker/volumes/oradata/_data";
  ...
  private static final GenericContainer<?> ORACLE_XE = new GenericContainer<>(DOCKER_IMAGE_ORACLE)
    ...
    .withFileSystemBind(DIRECTORY_HOST_ORACLE_DATA, DIRECTORY_CONTAINER_ORACLE_DATA)
    .withFileSystemBind(oracleSetupDirectory(), DIRECTORY_CONTAINER_ORACLE_SETUP)
  ...
  private static String oracleSetupDirectory()
  {
    try
    {
      File oracleDataDirectory = new File(DIRECTORY_PROJECT_ROOT, DIRECTORY_HOST_ORACLE_SETUP);
